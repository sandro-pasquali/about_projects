...
ig.Entity = ig.Class.extend({
    ...
    this.initSettings = settings; // framechange - for resurrection
		ig.merge( this, settings );
    this.emit("owner:init"); // framechange
	},

  // framechange - added
  isAlive: function() { return this._killed !== true; },

  // framechange - added
  setChangeableSetting: function(settings, setting, value) {
  ...
  // framechange - added
  setDefault: function(toSet, value, defaultValue) {
    toSet = value;
    if(toSet === undefined) {
      toSet = defaultValue
    }
  ...
  // framechange
  getCenter: function() { return ig.maths.center(this); },
  getPosition: function() { return this.pos; },
  getSize: function() { return this.size; },

  // framechange
  setupBehaviour: function(settings) {
    var behaviours = this.extractBehaviourDefs(settings);
    if(behaviours.length > 0) {
      ig.andro.setup(this);
    }

    for(var i = 0; i < behaviours.length; i++) {
      ig.andro.augment(this, behaviours[i]);
    }
  },

  // framechange
  extractBehaviourDefs: function(settings) {
    var behaviours = [];
    for(var settingName in settings) {
      if(settingName.match(/^behaviour.*/)) {
        var behaviour = window[settings[settingName]];
        if(behaviour !== undefined) {
          behaviours.push(behaviour);
        }
        else {
          console.warn("Block trying to use undefined behaviour: " + settings[settingName]);
        }
      }
    }
    return behaviours;
  ...
  // framechange
  movingX: function() {
    return Math.abs(this.body.m_linearVelocity.x) > 5
  },

  // framechange
  movingY: function() {
    return Math.abs(this.body.m_linearVelocity.y) > 5;
  },

  // framechange - added
  emit: function(event, data) {
    if(ig.andro.isSetup(this)) {
      ig.andro.eventer(this).emit(event, data);
    }
  },

  // framechange - added
  collision: function(selfShape, otherShape, contactType) {
    if(otherShape !== undefined && otherShape.isSensor !== true) {
      this.emit("owner:collision", {
        selfEntity: selfShape.GetBody().entity,
        otherEntity: otherShape.GetBody().entity,
  ...
  isInView: function() {
    ...
    this.emit("owner:update"); // framchange - added
	},
    ...
    this.emit("owner:draw"); // framchange - added
	},

	kill: function() {
		ig.game.removeEntity( this );
    this.emit("owner:kill"); // framchange - added
	},
